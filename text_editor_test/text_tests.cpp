...
TEST(text, can_find_text_2)
{
    ...
    EXPECT_EQ(text.GetRoot()->GetNextLevel()->GetNextLevel()->
	      GetNextLevel(), tmp);
}
...
TEST(text, can_copy_text_2_without_throws)
{
    TText text("Hello world");
    TTree *tmp =
	text.GetRoot()->GetNextLevel()->GetNextLevel()->GetNextLevel();
...
TEST(text, can_copy_text_2)
{
    ...
    EXPECT_EQ(tmp_2->GetNextLevel()->GetNextLevel()->GetNextLevel()->
	      GetLetter(), 'H');
    EXPECT_EQ(tmp_2->GetNextLevel()->GetNextLevel()->GetNextLevel()->
