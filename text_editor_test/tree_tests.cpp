...
TEST(tree, can_add_trees_correct)
{
    ...
    EXPECT_EQ(word1.GetSameLevel()->GetNextLevel()->GetSameLevel()->
	      GetLetter(), 'i');
}
...
TEST(tree, can_add_letter_to_tree_correct)
{
    ...
    text1.SetNextLevel(&string);
    text1 += letter;
    EXPECT_EQ(word1.GetNextLevel()->GetSameLevel()->GetSameLevel()->
	      GetLetter(), 'D');
}
...
TEST(tree, can_create_copy_of_tree)
{
    ...
    EXPECT_EQ(copy.GetNextLevel()->GetNextLevel()->GetNextLevel()->
	      GetLetter(), 'h');
    EXPECT_EQ(copy.GetNextLevel()->GetNextLevel()->GetNextLevel()->
	      GetSameLevel()->GetLetter(), 'e');
}
